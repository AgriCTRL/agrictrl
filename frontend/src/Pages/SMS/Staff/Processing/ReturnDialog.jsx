    ...
    const handleReturn = async () => {
        ...
        try {
            ...
            const facilityData =
                viewMode === "drying"
                    ? dryerData.find(
                          (dryer) => dryer.id === selectedItem.toLocationId
                      )
                    : millerData.find(
            ...
            const initialQuantity =
                viewMode === "drying"
                    ...
                    : parseInt(
                          selectedItem.driedQuantityBags ||
                              selectedItem.palayQuantityBags
            ...
            const facilityUpdateResponse = await fetch(
                `${apiUrl}/${
                    viewMode === "drying" ? "dryers" : "millers"
                }/update`,
                {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
            ...
            console.log(
                "target warehouse stock: ",
                targetWarehouse.currentStock
            );
            ...
            <div className="flex flex-col items-center gap-2">
                {selectedItem && (
                    <div className="flex items-center gap-2">
                        <label
                            htmlFor="wsr"
                            className="block text-base md:text-xl font-semibold text-black"
                        >
                            WSR:
                        </label>
                        <InputText
        ...
        const processedData =
            viewMode === "drying"
                ? {
                      ...
                      startDateTime:
                          selectedItem.fullProcessingBatchData?.dryingBatch
                              ?.startDateTime,
                      endDateTime:
                          selectedItem.fullProcessingBatchData?.dryingBatch
                              ?.endDateTime,
