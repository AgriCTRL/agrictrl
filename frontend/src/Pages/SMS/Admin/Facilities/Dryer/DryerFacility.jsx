...
function DryerFacility() {
  ...
  const fetchDryerData = async () => {
    try {
      ...
      toast.current.show({
        severity: "error",
        summary: "Error",
        detail: "Failed to fetch dryers data",
        life: 3000,
      });
    }
  };
  ...
  const handleDryerUpdated = (updatedDryer) => {
    const updatedData = dryerData.map((dryer) =>
      dryer.id === updatedDryer.id ? updatedDryer : dryer
    );
  ...
  const getRowClassName = (data) => {
    ...
    return remainingPercentage <= CAPACITY_WARNING_THRESHOLD
      ? "bg-red-100"
      : "";
  };
