...
function Transporter() {
  ...
  const [displayTransporterRegister, setDisplayTransporterRegister] =
    useState(false);
  const [displayTransporterUpdate, setDisplayTransporterUpdate] =
    useState(false);
  ...
  const fetchTransporterData = async () => {
    try {
      const res = await fetch(`${apiUrl}/transporters/user/${user.id}`);
      if (!res.ok) {
      ...
      toast.current.show({
        severity: "error",
        summary: "Error",
        detail: "Failed to fetch transporter data",
        life: 3000,
      });
    }
  };
  ...
  const handleTransporterUpdated = (updatedTransporter) => {
    const updatedData = transporterData.map((transporter) =>
      transporter.id === updatedTransporter.id
        ? updatedTransporter
        : transporter
    );
